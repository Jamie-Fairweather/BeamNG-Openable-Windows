...
local function updateGFX(dt) -- ms 
  ...
  electrics.values.LWindow = math.min(0, math.max(-1, electrics.values.LWindow + electrics.values.LWindowPress * 0.3 * dt))
  electrics.values.RWindow = math.min(0, math.max(-1, electrics.values.RWindow + electrics.values.RWindowPress * 0.3 * dt))
end
